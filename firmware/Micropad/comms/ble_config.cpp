...
void BLEConfigService::begin(ProtocolHandler* handler) {
    ...
    _cmdChar->setCallbacks(new NimBLECharacteristicCallbacks());
    _cmdChar->onWrite([](NimBLECharacteristic* pChar) {
        if (_instance) {
            _instance->handleWrite(pChar);
        }
    });
    ...
    _bulkChar->onWrite([](NimBLECharacteristic* pChar) {
        if (_instance) {
            _instance->handleWrite(pChar);
        }
    });
